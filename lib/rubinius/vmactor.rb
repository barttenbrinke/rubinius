...
module Rubinius
  class VMActor
    ...
    class << self
      ...
      def unserialize_message(value)
        ...
        when Rubinius::Tuple
          if value.first == :VMActor
            ...
            Rubinius::VMActor.new(vm_id, actor_id)
          else
      ...
      def spawn(container, remote_actor, *operands)
        ...
        when String
          ...
          Rubinius::Actor.spawn { eval command }
        when Symbol
          klass, meth, *args = operands
          klass = Object.const_lookup klass

          Rubinius::Actor.spawn { klass.send(meth, *args) }
        else
    ...
    def <<(value)
      Rubinius::VM.send_message(@container_id, [:actor, :message, @actor_id, Rubinius::VMActor.serialize_message(value)])
    end
    ...
    class Container
      ...
      class << self
        ...
        def process_messages
          Rubinius::VM.each_message do |command, *operands|
            Rubinius::VMActor.dispatch(*operands) if command == :actor
          end
        end
      end
      ...
      def initialize(*args)
        ...
        Rubinius::VMActor.register Actor.current

        # Ensure any -r's passed happen before we execute the message processor
        args += [
          "-rvmactor",
          "-e", "VMActor::Container.child_message_responder(#{Rubinius::VM_ID},#{Rubinius::Actor.current.object_id})",
        ]
      ...
      def spawn_actor(*args)
        Rubinius::VMActor.register Rubinius::Actor.current

        Rubinius::VM.send_message(@vm.id, [
          :actor,
          :spawn,
          Rubinius::VM_ID,
          Rubinius::Actor.current.object_id,
          *args
        ])
        ...
        Rubinius::Actor.receive do |filter|
          filter.when(Rubinius::Tuple[:spawn_reply, Object]) do |message|
            _, actor_id = message
            return Rubinius::VMActor.new(@vm.id, actor_id)
          end
